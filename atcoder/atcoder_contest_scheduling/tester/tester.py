...
with open("vis.svg", mode="w") as f:
    ...
    for i in range(26):
        r, g, b = color(c[i] / 100)
        f.write(
            '<rect x="{}" y="{}" width="40" height="10" fill="#{:02x}{:02x}{:02x}" stroke="grey" stroke-width="1"/>'.format(
                i * 40 + 1, 1, r, g, b
            )
        )
        f.write(
            '<text x="{}" y="{}" font-size="7">{}</text>'.format(i * 40 + 10, 9, c[i])
        )
    for d in range(D):
        for i in range(26):
            ...
            f.write(
                '<rect x="{}" y="{}" width="40" height="10" fill="#{:02x}{:02x}{:02x}" stroke="grey" stroke-width="1"/>'.format(
                    i * 40 + 1, 20 + 10 * d + 1, r, g, b
                )
            )
            f.write(
                '<text x="{}" y="{}" font-size="7">{}</text>'.format(
                    i * 40 + 10, 29 + 10 * d, s[d][i]
                )
            )
    for d in range(D):
        f.write(
            '<rect x="{}" y="{}" width="40" height="10" fill="none" stroke="black" stroke-width="3"/>'.format(
                (out[d] - 1) * 40 + 1, 20 + 10 * d + 1
            )
        )
    f.write("</svg>")
    f.write("</svg>")
